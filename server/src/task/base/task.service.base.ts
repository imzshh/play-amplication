...
import { Prisma, Task, User, Milestone } from "@prisma/client";
...
export class TaskServiceBase {
  ...
  async getAssignee(parentId: string): Promise<User | null> {
  ...
  async getMilestone(parentId: string): Promise<Milestone | null> {
    return this.prisma.task
      .findUnique({
        where: { id: parentId },
      })
